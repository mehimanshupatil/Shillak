...
function recapFor(year: number, month: number) {
  return computeMonthlyRecap(ledgerFrom(transactions, 'INR'), budgets, goals, { year, month })
}
...
describe('computeMonthlyRecap', () => {
  ...
  it('only includes categories that have a budget set', () => {
    transactions.push(
  ...
  it('returns an empty budgets array when the group has no budgets at all', () => {
    transactions.push(makeTxn({ categoryId: 'cat-1', amount: 5000 }))
    const result = recapFor(2026, 5)
    expect(result.budgets).toHaveLength(0)
  })

  it('ranks top categories by spend, capped at 5', () => {
    transactions.push(
  ...
  it('computes this-month delta for a categoryId-linked (auto-tracked) goal', () => {
    transactions.push(
  ...
  it('reports zero delta for a manually-tracked (no categoryId) goal', () => {
    goals.push({
      goalId: 'goal-2',
      groupId: 'g1',
      name: 'Goa trip',
