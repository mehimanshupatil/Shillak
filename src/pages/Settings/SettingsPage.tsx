    ...
    <div className="px-4 pt-6 pb-24 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {group && (
            <Button
              ...
              className="text-[var(--color-accent)] hover:text-[var(--color-accent)] hover:bg-[var(--color-accent-subtle)] gap-1"
            >
              <Pencil size={11} />
              Edit
            </Button>
        ...
        {group ? (
          <div className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] divide-y divide-[var(--color-border)]">
            ...
            <div className="px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-[var(--color-text-secondary)]">Split bills</span>
              <span
                className={`text-xs font-medium ${group.splitEnabled ? 'text-[var(--color-success)]' : 'text-[var(--color-text-tertiary)]'}`}
              >
                {group.splitEnabled ? 'On' : 'Off'}
              </span>
            </div>
            <div className="px-4 py-3 flex items-center justify-between">
              <span className="text-sm text-[var(--color-text-secondary)]">Income tracking</span>
              <span
                className={`text-xs font-medium ${group.incomeTracking ? 'text-[var(--color-success)]' : 'text-[var(--color-text-tertiary)]'}`}
              >
          ...
          <div className="h-20 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] animate-pulse" />
        )}
      </section>
        ...
        <div className="flex flex-col gap-1.5">
          {filteredCats.map((cat) => (
            ...
            >
              ...
              {cat.isDefault && (
                <span className="text-[10px] text-[var(--color-text-tertiary)] bg-[var(--color-surface-2)] px-1.5 py-0.5 rounded">
                  default
                </span>
              )}
              <Button
        ...
        <div className="flex flex-col gap-1.5">
          {(members ?? []).map((member) => (
            ...
            >
              ...
              {member.role === 'admin' && (
                <Crown size={13} className="text-[var(--color-accent)]" />
              )}
      ...
      <section>
        ...
        {user ? (
          ...
          <div className="h-16 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] animate-pulse" />
        )}
      </section>
      ...
      <section>
        ...
        <div className="rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] divide-y divide-[var(--color-border)] overflow-hidden">
          <button
            type="button"
          ...
          >
            <span className="text-sm text-[var(--color-text-primary)]">Import snapshot</span>
        ...
        <input
          ...
          className="hidden"
        />
        {importMsg && <p className="text-xs text-[var(--color-success)] px-1 mt-1">{importMsg}</p>}
      ...
      <section>
        ...
        <div className="rounded-2xl bg-surface border border-[var(--color-border)] divide-y divide-[var(--color-border)] overflow-hidden">
          <button
            type="button"
            onClick={() => setChangePinOpen(true)}
            className="w-full flex items-center justify-between px-4 py-3 transition-colors hover:bg-[var(--color-surface-2)]"
          >
